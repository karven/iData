...
-(void)loadFile{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        for (NSString *fileName in array) {
            ...
            if ([type isEqualToString:@"NSFileTypeDirectory"] || [suffix isEqualToString:@"rtfd"] || [suffix isEqualToString:@"txt"] || [suffix isEqualToString:@"doc"] || [suffix isEqualToString:@"docx"] || [suffix isEqualToString:@"xls"] || [suffix isEqualToString:@"xlsx"] || [suffix isEqualToString:@"ppt"] || [suffix isEqualToString:@"pptx"] || [suffix isEqualToString:@"pdf"]) {
    ...
    if (tag!=-1) {
        ...
        if ([self.isStatus isEqualToString:@"scan"]) {
            if ([type isEqualToString:@"NSFileTypeDirectory"]) {
                FileViewController *videoVC = [[FileViewController alloc] init];
                    ...
                    for (NSDictionary *fileInfo in kFTPManager.selectFileArray) {
                        ...
                        if ([nsFileType isEqualToString:@"NSFileTypeDirectory"]) {
                            ...
                            if ([fileManager fileExistsAtPath:destFilePath isDirectory:&isflag]) {
                                ...
                                BOOL flag = [Util isPasteFile:nsFilePath dstType:@"document"];
                                if (flag) {
                                    [FileCopy copyFile:nsFilePath toPath:destFilePath];
                                }else{
                                    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"只能复制文档类文件" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                                    [alertView show];
                                }
                            }else{
                                BOOL flag = [Util isPasteFile:nsFilePath dstType:@"document"];
