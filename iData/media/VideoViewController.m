...
-(void)loadFile{
    ...
    dispatch_async(dispatch_get_global_queue(0,0), ^{
        ...
        if (ALAssetsLibrary.authorizationStatus == ALAuthorizationStatusAuthorized && [[dirPath substringToIndex:dirPath.length-1] isEqualToString:VideoRootDirectory]) {
            [_fileList addObject:[NSMutableDictionary dictionaryWithObjectsAndKeys:@"视频库",@"NSFileName",@"NSFileTypeDirectory",@"NSFileType", nil]];
        }
        ...
        for (NSString *fileName in array) {
            ...
            if ([type isEqualToString:@"NSFileTypeDirectory"] || [suffix isEqualToString:@"rmvb"] || [suffix isEqualToString:@"rm"] || [suffix isEqualToString:@"wmv"] || [suffix isEqualToString:@"avi"] || [suffix isEqualToString:@"mov"] || [suffix isEqualToString:@"mpeg"] || [suffix isEqualToString:@"asf"] || [suffix isEqualToString:@"mp4"] || [suffix isEqualToString:@"flv"]) {
    ...
    if (tag!=-1) {
        ...
        if ([self.isStatus isEqualToString:@"scan"]) {
            if ([type isEqualToString:@"NSFileTypeDirectory"]) {
                if (indexInFileList == 0 && ALAssetsLibrary.authorizationStatus == ALAuthorizationStatusAuthorized && [fileName isEqualToString:@"视频库"]) {
                    VideoVistViewController *vistVC = [[VideoVistViewController alloc] init];
                ...
                NSString *videoPath = [fileDicInfo objectForKey:@"NSFilePath"];
                FileContentViewController *file = [[FileContentViewController alloc] init];
                file.fileName = videoPath;
                [self.navigationController pushViewController:file animated:YES];
            }
        }else{
            if (ALAssetsLibrary.authorizationStatus == ALAuthorizationStatusAuthorized && [fileName isEqualToString:@"视频库"] && indexInFileList == 0) {
                [self.view makeToast:@"mediaLibrary" duration:3 position:[NSValue valueWithCGPoint:CGPointMake(ScreenWidth/2,ScreenHeight-74)]];
                [self scanAction:nil];
                return;
            }
                    ...
                    for (NSDictionary *fileInfo in kFTPManager.selectFileArray) {
                        ...
                        if ([nsFileType isEqualToString:@"NSFileTypeDirectory"]) {
                            ...
                            if ([fileManager fileExistsAtPath:destFilePath isDirectory:&isflag]) {
                                ...
                                BOOL flag = [Util isPasteFile:nsFilePath dstType:@"video"];
                                if (flag) {
