...
-(void)loadFile{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        for (NSString *fileName in array) {
            ...
            if ([type isEqualToString:@"NSFileTypeDirectory"] || [suffix isEqualToString:@"MP3"] || [suffix isEqualToString:@"WMA"] || [suffix isEqualToString:@"WAV"] || [suffix isEqualToString:@"OGG"] || [suffix isEqualToString:@"AC3"] || [suffix isEqualToString:@"AIFF"] || [suffix isEqualToString:@"DAT"]) {
    ...
    if (tag!=-1) {
        ...
        if ([self.isStatus isEqualToString:@"scan"]) {
            if ([type isEqualToString:@"NSFileTypeDirectory"]) {
                ...
                NSString *musicPath = [fileDicInfo objectForKey:@"NSFilePath"];
                //添加后台播放代码：
                ...
                NSError *playerError;
                //播放
                NSURL *url = [NSURL fileURLWithPath:[musicPath stringByAppendingString:fileName]];
                player = nil;
                    ...
                    for (NSDictionary *fileInfo in kFTPManager.selectFileArray) {
                        ...
                        if ([nsFileType isEqualToString:@"NSFileTypeDirectory"]) {
                            ...
                            if ([fileManager fileExistsAtPath:destFilePath isDirectory:&isflag]) {
                                ...
                                BOOL flag = [Util isPasteFile:nsFilePath dstType:@"music"];
                                if (flag) {
//                                    [FileCopy copyFile:nsFilePath fileName:nsFileName toPath:dstPath];
                                    [FileCopy copyFile:nsFilePath toPath:destFilePath];
                                }else{
                                    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"只能复制音乐类文件" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                                    [alertView show];
                                }
                            }else{
                                BOOL flag = [Util isPasteFile:nsFilePath dstType:@"music"];
...
//创建文件夹，并把src中文件拷贝到当前创建的文件夹中
-(void)createDir:(NSString *)dirPath srcPath:(NSString *)src srcName:(NSString *)name{
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *otherDirPath = [NSString stringWithFormat:@"%@/%@",dirPath,name];
    [fileManager createDirectoryAtPath:otherDirPath withIntermediateDirectories:YES attributes:nil error:nil];
    NSString *filePath = [NSString stringWithFormat:@"%@/%@",src,name];
    NSArray *array = [fileManager contentsOfDirectoryAtPath:filePath error:nil];
    for (NSString *fileName in array) {
        NSString *otherFilePath = [NSString stringWithFormat:@"%@/%@",filePath,fileName];
        NSDictionary *fileInfo = [fileManager attributesOfItemAtPath:otherFilePath error:nil];
        NSString *fileType = [fileInfo objectForKey:@"NSFileType"];
        if ([fileType isEqualToString:@"NSFileTypeDirectory"]) {
            NSString *dir = [NSString stringWithFormat:@"%@/%@",dirPath,fileName];
            [self createDir:dir srcPath:otherFilePath srcName:fileName];
        }else{
            [fileManager copyItemAtPath:otherFilePath toPath:otherDirPath error:nil];
        }
    }
}
...
-(void)deleteAction{
    ...
    for (NSDictionary *dic in selectFileArray) {
        NSString *path = [dic objectForKey:@"NSFilePath"];
        NSString *name = [dic objectForKey:@"NSFileName"];
        BOOL flag = [fileManager removeItemAtPath:[path stringByAppendingString:name] error:nil];
        if (flag) {
