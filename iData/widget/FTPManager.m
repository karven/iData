...
static FTPManager *sigleton = nil;

+(FTPManager *)shareInstance{
    if (sigleton == nil) {
        @synchronized(self){
            sigleton = [[FTPManager alloc] init];
            sigleton.folderArray = [NSMutableArray array];
            sigleton.selectFileArray = [NSMutableArray array];
        }
    }
    return sigleton;
}
...
@synthesize password, username, destination, port/*, destinationArray*/;
- (id)init
{
    self = [super init];
    if (self) {
...
static FMServer *FMServerSigleton = nil;

+(FMServer *)shareFMServerInstance{
    if (FMServerSigleton == nil) {
        @synchronized(self){
            FMServerSigleton = [[FMServer alloc] init];
            [FMServerSigleton setUsername:@"admin"];
            [FMServerSigleton setPassword:@"admin"];
        }
    }
    return FMServerSigleton;
}

//-(void)setHostDestination:(NSMutableArray *)array{
//    if (array.count > 0) {
//        NSMutableString *des = [NSMutableString stringWithString:FTPURL];
//        for (NSString *s in array) {
//            [des appendString:[NSString stringWithFormat:@"%@/",s]];
//        }
//        NSLog(@"%@",des);
//        [self setDestination:[des substringToIndex:des.length - 1]];
//    }
//}
